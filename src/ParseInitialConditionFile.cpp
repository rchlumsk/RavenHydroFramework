  ...
  while (!end_of_file)
  {
    ...
    */
      ...
      do
      {
        ...
        else if (!strcmp(s[0],":Qlat"))
        {
          if (Len>2){
            ...
            pBasin->SetQlatHist(histsize,aQlat,aQlat[histsize],rvc_tstep,Options.timestep);
            delete [] aQlat;
          }
        }
        else if (!strcmp(s[0],":Qin"))
        {
          if (Len>2){
            ...
            pBasin->SetQinHist(histsize,aQin,rvc_tstep,Options.timestep);
            delete [] aQin;
          }
        }
    ...
    case(18):  //----------------------------------------------
    {
      //:TimeStep [timestep]
      if (Len<2){break;}
      rvc_tstep=s_to_d(s[1]);
      if (fabs(rvc_tstep-Options.timestep)>REAL_SMALL){
        WriteAdvisory("ParseInitialConditionsFile: RVC file generated by model with different simulation time step. Inflow and lateral flow histories have been rescaled.",Options.noisy);
      }
      break;
    }
    case(19):  //----------------------------------------------
