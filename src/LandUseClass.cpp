...
//const surface_struct &needed_params
{
  ...
  autocalc=SetCalculableValue(S.firn_melt_corr,Stmp.firn_melt_corr,Sdefault.firn_melt_corr);
  if (autocalc)
  {
    S.firn_melt_corr =1.0;  //Default - no firn correction
    //no warning
  }
  autocalc=SetCalculableValue(S.firn_compaction_rate,Stmp.firn_compaction_rate,Sdefault.firn_compaction_rate);
  if (autocalc)
  {
    S.firn_compaction_rate =1.3;  //Default - %/yr
    warn="The required parameter FIRN_COMPACTION_RATE for land use class "+S.landuse_name+" was autogenerated with value "+to_string(S.firn_compaction_rate);
    if(chatty) { WriteAdvisory(warn,false); }
  }
  ...
  if (autocalc){
    ...
    warn="The required parameter UBC_ICEPT_FACTOR for land use class "+S.landuse_name+" was autogenerated with value "+to_string(S.UBC_icept_factor);
    if(chatty) { WriteAdvisory(warn,false); }
  }

  autocalc = SetCalculableValue(S.sky_view_factor,Stmp.sky_view_factor,Sdefault.sky_view_factor);
  if(autocalc) {
    S.sky_view_factor = 0.8; //80% of hemisphere  is visible from location --> 1.0 for really flat terrain
    warn="The required parameter SKY_VIEW_FACTOR for land use class "+S.landuse_name+" was autogenerated with value "+to_string(S.sky_view_factor);
    if(chatty) { WriteAdvisory(warn,false); }
  }
  ...
  autocalc = SetCalculableValue(S.max_wind_speed,Stmp.max_wind_speed,Sdefault.max_wind_speed);
  if(autocalc) {
    S.max_wind_speed = 8.0; //m/s
    warn="The required parameter MAX_WIND_SPEED for land use class "+S.landuse_name+" was autogenerated with value 8.0 m/s";
    if(chatty) { WriteAdvisory(warn,false); }
  }
  autocalc = SetCalculableValue(S.stream_fraction,Stmp.stream_fraction,Sdefault.stream_fraction);
  if(autocalc) {
    S.stream_fraction = 0.0;
  ...
  SetSpecifiedValue(S.snow_dampen_coeff,Stmp.snow_dampen_coeff,Sdefault.snow_dampen_coeff,needed,"SNOW_DAMPEN_COEFF");
...
void CLandUseClass::InitializeSurfaceProperties(string name, surface_struct &S, bool is_template)
{
  ...
  S.firn_melt_corr   =DefaultParameterValue(is_template,true);
  S.firn_compaction_rate=DefaultParameterValue(is_template,true);
                                        ...
                                        const double value)
{
  ...
  else if (!name.compare("FIRN_MELT_CORR"         )){S.firn_melt_corr=value;}
  else if (!name.compare("FIRN_COMPACTION_RATE"   )){S.firn_compaction_rate=value;}
  ...
  else if (!name.compare("SNOW_DAMPEN_COEFF"      )){S.snow_dampen_coeff=value;}
...
double CLandUseClass::GetSurfaceProperty(const surface_struct &S, string param_name, const bool strict)
{
  ...
  else if (!name.compare("FIRN_MELT_CORR"         )){return S.firn_melt_corr;}
  else if (!name.compare("FIRN_COMPACTION_RATE"   )){return S.firn_compaction_rate;}
  ...
  else if (!name.compare("SNOW_DAMPEN_COEFF"      )){return S.snow_dampen_coeff; }
  ...
  else if (!name.compare("MIN_WIND_SPEED"         )){return S.min_wind_speed; }
